...
generate.plots = function(fullDataset, minEstimator=FALSE, useC=FALSE, iteratedC=FALSE){
	samples = fullDataset;
	...
	experimentName = NULL;
	if(is.character(minEstimator)){
		experimentName = minEstimator;
	} else if(useC) {
		experimentName = "Estimate c";
	} else if(iteratedC){
		experimentName = "Iterated estimator";
	} else {
		experimentName = "No estimate c";
	}
	...
	for(i in 1:length(models)){
		...
		df = rbind(df, c(experimentName, model$name, paste.vector(params),
						 -2*retval$cross,
	...
	outputName = paste(experimentName, "-", sampleSize, sep="");
