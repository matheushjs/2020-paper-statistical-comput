...
difference = function(params){
	...
	-sum(log(mydist(slice, shape=params[1], scale=params[2])) * dgamma(slice, shape=realParams[1], scale=realParams[2]));
}
...
# Histogram
hist(data, freq=FALSE, border=F, col="#A0A0A070", xlab="x", ylab="density", xlim=c(0, 100), ylim=c(0, 0.035), axes=FALSE);
axis(1, pos=0);
axis(2, pos=0);
...
# Real dist
lines(x, dgamma(x, shape=realParams[1], scale=realParams[2]) / pgamma(6*pi, shape=realParams[1], scale=realParams[2], lower.tail=FALSE), lwd=mylwd[1], lty=mylty(1), col=mycolors[1]);
