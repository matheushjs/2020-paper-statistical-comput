...
for(N in ALL.N){
	for(idx in 1:ITERATIONS){
		...
		estim[[2]] = data$min - abs(data$min) * data$stddev / N;
		estim[[3]] = data$min - abs(data$min) * data$stddev * sqrt(log(log(N)) / (2*N));
		estim[[4]] = data$min - abs(data$min) * data$stddev * sqrt(-log(0.05/2) / (2*N));
...
colnames(df) = c("N", paste("dist", 1:4, sep=""), paste("lik", 1:4, sep=""), "likBase");
#print(df);
...
#plot(ALL.N, plotMeans[,5]   - base, type="l", ylim=c(-0.05, 1), col=1, log="x", xlab="", ylab="");
#lines(ALL.N, plotMeans[,6]  - base, col=2, lty=2);
#lines(ALL.N, plotMeans[,7]  - base, col=3, lty=3);
#lines(ALL.N, plotMeans[,8]  - base, col=4, lty=4);
...
legend("topright", c(expression(textstyle(min)), expression(c2), expression(c3), expression(c4)), col=1:4, lwd=3, lty=1:4, seg.len=4, bg="#FFFFFFBB");
